...
# --- Alignment overrides for page 2 ---

st.markdown("""
<style>
/* === Centering & single-line icon row for '추천 정보' (page 2) === */

/* Tag container as a single centered row (scroll if overflow) */
.meta-icons {
  display: flex;
...
/* Make pills look consistent inside the row */
.meta-icons .pill {
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
/* Title & metadata center alignment (override earlier left rules) */
.card h3, .card .meta, .card .title-sub, .card .shop-link-wrap, .card .card-footer {
  text-align: center !important;
  margin-left: auto !important;
  margin-right: auto !important;
}
.card h3 { letter-spacing: .01em !important; }

/* Make Streamlit columns hold cards centered */
[data-testid="column"] {
  display: flex !important;
  flex-direction: column !important;
...
/* Keep product button centered */
.shop-link-wrap { text-align: center !important; }

/* Tighter rhythm between elements */
.card img { margin: .15rem auto !important; }
.meta-icons { margin-top: .2rem !important; }
.card-footer { margin-top: .35rem !important; padding-top: .25rem !important; }

/* Optional: keep "추천 이유" pills centered when expanded */
[data-testid="stExpander"] div[role="region"] { text-align: center; }
</style>
""", unsafe_allow_html=True)
